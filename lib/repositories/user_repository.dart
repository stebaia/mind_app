  ...
  Future<User> login(
    ...
    try {
      final response = await userService
          .login(LoginRequest(email: sha256.convert(utf8.encode(email)).toString(), password: sha256.convert(utf8.encode(password)).toString(), timestamp: DateConverter.getDateNowWithFormat()));
